...
Dense and BM25 apply the same authorization filters while building candidates. PostgreSQL owns document, chunk, and authorization truth; Qdrant stores rebuildable vectors. Final citations can refer only to authorized chunks actually supplied to the model.
...
- Ordinary memory uses Qdrant global vector recall by default and merges it with a bounded PostgreSQL fallback. Repeated calls in one turn deduplicate by Memory ID before count and token budgets are applied.
...
- Every candidate is matched through exact hash, canonical key/category, Qdrant, and PostgreSQL fallback retrieval, then sent to a mandatory second Memory Judge for `independent/equivalent/refinement/replacement/uncertain/discard`; the service maps that relation to the final write action.
...
- PostgreSQL remains authoritative. Qdrant synchronization is best effort, and Celery Beat repairs missing or stale memory points daily.
...
3. **Post-answer memory update:** stage one extracts candidates only; the system retrieves related Qdrant/PostgreSQL memories for each candidate; stage two adjudicates every candidate before deterministic backend rules and a database transaction may persist it.
...
| Engineering quality gate | Production-container dependencies and real Qdrant | **310/310** backend regressions, 24 Alembic revisions, production frontend build, and end-to-end smoke all passed | Reliability and deployability |
...
`evaluate_rgb_agent_runtime.py` and `evaluate_longmemeval_agent_runtime.py` exercise the real production Agent/database/Qdrant path. They require an isolated evaluation database and a dedicated Qdrant collection. Never run evaluation seeding against production data.
...
`EMBEDDING_DIMENSION` must match the model output. Changing the model or dimension requires rebuilding the corresponding Qdrant collection. Document vectors must be re-ingested; Memory points can be regenerated by reconciliation, but old-dimension vectors cannot be reused.
...
| `MEMORY_VECTOR_INDEX_ENABLED` | true | Qdrant is used for answer recall and pre-update related-memory recall |
...
The gate currently runs 310 backend regression tests, Python `compileall`, full Alembic migration verification, the TypeScript/Vite build, and development/production Compose validation. The 2026-07-15 run passed **310/310** with production-container dependencies and real Qdrant. GitHub Actions additionally builds the production frontend image, runs the Nginx parameter preflight, and executes `nginx -t`. Add the end-to-end smoke demo when the stack is running:
...
- The Qdrant Memory index is a best-effort derivative. Failure falls back to a bounded PostgreSQL window, so a very old ordinary memory outside that window may be missed.
