...
def summarize_with_rag(db: Session, state: AgentGraphState) -> AgentGraphState:
    ...
    source_context = format_answer_context(evidence.chunks)
    ...
    completion = get_llm_provider().summarize_with_metadata(
        source_context,
        request_text=state.input,
    ...
    add_trace(
        state,
        node="summary_agent",
        action="summarize_rag_evidence",
        input_data={"evidence_chars": len(source_context), "chunk_count": len(evidence.chunks)},
        output_data={
            "summary_chars": len(state.answer),
