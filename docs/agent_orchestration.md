...
中间件只保留模型每回合返回的第一个工具调用，并设置 `parallel_tool_calls=false`。默认配置是 6 次模型调用、4 次总工具调用，其中 Memory 2 次、RAG 3 次；最后一次模型调用不再提供工具，必须基于已有信息回答或明确说明证据不足。模型调用上限是硬限制；工具总预算与分预算目前在下一次模型调用前通过解绑工具实施，工具执行入口尚无第二道硬校验。

每次调用工具前，模型必须确认一个具体且尚未解决的信息需求；所有必要事实已经被当前上下文直接覆盖时立即回答，不得为了确认、增加信心或收集更多引用而继续检索。一次无进展结果只允许使用实质不同的 query 重试一次，第二次仍无进展时应停止该工具。该无进展停止策略目前由 Prompt 引导；服务端硬性拒绝完全重复 query，并对模型调用实施硬上限，对工具预算执行调用前解绑。

过滤后的最近 user/assistant 对话以真实 `HumanMessage` / `AIMessage` 传入模型，不再拼进 Memory 文本块。工具调用结果在当前运行内保留：`ToolMessage` 只携带状态、数量和日志 ID 等轻量回执，完整 Memory/RAG 内容分别由动态用户上下文和累计 RAG 证据提供，避免同一内容重复注入。工具消息不会写入跨轮 `Conversation.Message` 历史。
...
- 用户记忆和企业证据严格分离。
- 权限、超时、完全重复 query、模型调用上限、检索 provenance 与最终收口由后端执行；工具分预算目前缺少执行入口二次硬校验。
